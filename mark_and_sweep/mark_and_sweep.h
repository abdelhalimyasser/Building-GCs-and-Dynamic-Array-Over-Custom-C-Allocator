#ifndef MARK_AND_SWEEP_H_INCLUDED
#define MARK_AND_SWEEP_H_INCLUDED
...
typedef struct VM
{
    Object *first_object; // Pointer to the start of the allocated objects list
...
/**
 * @brief Executes the Mark and Sweep Garbage Collection algorithm.
 *
 * @details Traverses all objects starting from the roots, utilizing pointer tagging
 * to mark reachable objects. It then sweeps the heap, safely freeing the memory
 * of any unreachable objects and untagging the reachable ones for the next cycle.
 *
 * **Complexity:**
 * - Time Complexity: O(R + N), where R is the number of roots and N is the total number of objects.
 * - Space Complexity: O(D) where D is the maximum depth of the object graph (due to call stack recursion).
 *
 * @param vm Pointer to the Virtual Machine state.
 * @return void
...
#endif // MARK_AND_SWEEP_H_INCLUDED